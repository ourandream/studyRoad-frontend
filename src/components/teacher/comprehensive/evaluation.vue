      ...
      >
        <el-col :span="6">
          <el-form-item label="年级" prop="zhgrade">
            <el-select
              v-model="zhFind.zhgrade"
              placeholder="请选择年级"
              clearable
              :style="{ width: '100%' }"
            >
              <el-option
                v-for="(item, index) in zhgradeOptions"
                :key="index"
                :label="item.label"
                :value="item.value"
        ...
        <el-col :span="6">
          <el-form-item label="学年" prop="zhscyear">
            <el-select
              v-model="zhFind.zhscyear"
              placeholder="请选择扣分学年"
              clearable
              :style="{ width: '100%' }"
            >
              <el-option
                v-for="(item, index) in zhscyearOptions"
                :key="index"
                :label="item.label"
                :value="item.value"
                :disabled="item.disabled"
...
export default {
  ...
  methods: {
    stuzcDetails(row, event, column) {
      this.$router.push({
        path: "/zhdetail",
        query: { stuid: row.stunum, scyear: this.zhFind.zhscyear },
      });
    },
