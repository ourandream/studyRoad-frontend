      ...
      <el-container>
        ...
        <el-form ref="changePs" :model="changePs" :rules="rules" size="medium" label-width="100px"
          ...
          <el-form-item label="账号" prop="id" style="padding-top: 3%">
            <el-input v-model="changePs.id" placeholder="请输入您的账号" clearable prefix-icon='el-icon-user-solid'
                      disabled
...
<script>
  ...
  import {changeStupa } from '../../api/Users'
  ...
  export default {
    ...
    created () {
      this.changePs.id=this.$session.get("user").stunum
    },
    methods:{
      async changeStuPa(painfo){
        const { data }=await changeStupa(painfo)
        var code=data.code
      ...
      submitForm(){
        this.$refs['changePs'].validate(valid => {
          if (!valid) return
          // TODO 提交表单
          this.changeStuPa(this.changePs)
          this.$refs['changePs'].resetFields()
        })
