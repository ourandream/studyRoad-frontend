    ...
    <el-container>
        ...
        <el-card class="bac">
          ...
          >
            ...
            <el-form-item label="打分同学" prop="stu">
              <el-select
                v-model="democratic.stu"
                placeholder="请选择打分同学"
                clearable
                :style="{ width: '100%' }"
              >
                <el-option
                  v-for="(item, index) in stuOptions"
                  :key="index"
                  :label="item.label"
                  :value="item.value"
      ...
      <el-container>
        ...
        >
          <h3>评分标准</h3>
          <img
            src="../../assets/img/Democraticappraisal.jpg"
            style="width: 100%"
          />
        </el-card>
      </el-container>
...
export default {
  ...
  methods: {
    ...
    async setdemocratic(score, scoreed, de) {
      ...
      if (code === 3009) this.$message.error(message);
      else if (code == 200)
        this.$message({
          message: "打分提交成功！",
          type: "success",
