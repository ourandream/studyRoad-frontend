  ...
  >
    ...
    >
      <el-form-item label="用户账号" prop="id">
        <el-input
          v-model="forgetData.id"
          placeholder="请输入账号用户账号"
          clearable
...
export default {
  ...
  methods: {
    async ForgetPa(forgetdata) {
      ...
      if (data.code === 3010) this.ForgetPa2(forgetdata);
      else {
        if (data.code === 200) {
          this.$message({
            showClose: true,
